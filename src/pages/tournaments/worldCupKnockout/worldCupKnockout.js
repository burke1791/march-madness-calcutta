...
function WorldCupKnockout(props) {
  ...
  const [bracket, bracketReturnDate, fetchBracket] = useData({
    ...
    endpoint: `${TOURNAMENT_SERVICE_ENDPOINTS.GET_WORLD_CUP_BRACKET}/${leagueId}`,
    method: 'GET',
  ...
  return (
    <Layout>
      ...
      <Content style={{ textAlign: 'center', minHeight: 'calc(100vh - 192px)', height: '100%' }}>
        <HighlightDropdown
          selectedUserKey='bracketUserSelected'
          selectPlaceholder='Highlight Teams'
        />
        <Row justify='center'>
          <Col span={24} style={{ overflow: 'auto' }}>
            {gamesLoading ? <Skeleton active /> : <BracketWrapper games={games} />}
          </Col>
        </Row>
      </Content>
    </Layout>
...
export default WorldCupKnockout;